...
**F1 news proxy.** `/f1-news` is rewritten to `https://www.formula1.com/en/latest/all.xml` both in dev (`vite.config.ts` server proxy) and in production (`vercel.json` rewrites) to avoid CORS — keep these two in sync if the upstream URL changes. [src/components/NewsIntel.tsx](src/components/NewsIntel.tsx) fetches the rewritten path, parses the RSS, and first tries to extract a per-item image from the feed itself (enclosure / `media:*` / inline `<img>`) — in practice the live feed carries none of these. As a backfill, it fetches each article's page through a second proxy, `/f1-article/:path*` → `https://www.formula1.com/:path*` (same dev/prod pairing, same CORS rationale), and scrapes the `og:image` meta tag, patching images into state as each one resolves.
